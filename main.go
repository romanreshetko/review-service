...
func main() {
	...
	mux.HandleFunc("/city/popular", h.GetPopularCitiesHandler)
	mux.HandleFunc("/city", h.GetCityByIDHandler)
	mux.HandleFunc("/review/popular", h.GetPopularReviewsHandler)
	mux.HandleFunc("/review/closest", h.GetClosestReviewsHandler)
	mux.HandleFunc("/review/region", h.GetReviewsByRegionHandler)
	...
	mux.Handle("/review/status", authMiddleware(http.HandlerFunc(h.GetReviewsByStatus)))
	mux.Handle("/review/user", authMiddleware(http.HandlerFunc(h.GetReviewsByTokenHandler)))
